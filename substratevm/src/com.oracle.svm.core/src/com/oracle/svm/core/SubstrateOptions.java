...
public class SubstrateOptions {
    ...
    public static class DeprecatedOptions {
        ...
        @APIOption(name = "G1", group = GCGroup.class, customHelp = "G1 garbage collector")//
        @Option(help = "Determines if the G1 garbage collector should be used.", type = User, deprecated = true, deprecationMessage = "Please use '--gc=G1' instead")//
        public static final HostedOptionKey<Boolean> UseG1GC = new HostedOptionKey<>(false) {
            @Override
            protected void onValueUpdate(EconomicMap<OptionKey<?>, Object> values, Boolean oldValue, Boolean newValue) {
                if (newValue) {
                    SubstrateOptions.SupportedGCs.update(values, ReplacingLocatableMultiOptionValue.DelimitedString.buildWithCommaDelimiter(GCOptionValue.G1.getValue()));
                } else if (((ReplacingLocatableMultiOptionValue.DelimitedString) values.get(SubstrateOptions.SupportedGCs)).contains(GCOptionValue.G1.getValue())) {
                    SubstrateOptions.SupportedGCs.update(values, ReplacingLocatableMultiOptionValue.DelimitedString.buildWithCommaDelimiter());
                }
            }
        };
